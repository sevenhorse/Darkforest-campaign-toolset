...
function initCalendarEngine() {
    ...
    timeFlowInterval = setInterval(() => {
        if (timeFlowActive) { universeTimeHours += 1; updateCalendarDisplay(); }
    }, 4000);
}
...
window.switchTermTab = function(tabName) {
    ...
    if (tabName === 'cargo') { if (typeof populateCargoVesselSelect === 'function') populateCargoVesselSelect(); if (typeof renderTerminalCargoDeck === 'function') renderTerminalCargoDeck(); } 
    else if (tabName === 'vessel') { if (typeof populateVesselDeckSelect === 'function') populateVesselDeckSelect(); if (typeof renderVesselDeck === 'function') window.renderVesselDeck(); } 
    else if (tabName === 'codex') { window.switchCodexCategory(activeCodexCategory); }
};
...
window.openFullCargoTerminal = function() { document.getElementById('character-terminal').style.display = 'block'; window.switchTermTab('cargo'); };
window.openFullCodexTerminal = function() { document.getElementById('character-terminal').style.display = 'block'; window.switchTermTab('codex'); };
window.openFullVesselTerminal = function(vesselId) {
    document.getElementById('character-terminal').style.display = 'block';
...
window.toggleCommsArray = function() { 
    ...
    if (panel.style.display === 'block') { if (typeof populateCommsRecipients === 'function') populateCommsRecipients(); if (typeof loadChatLogs === 'function') loadChatLogs(); }
};
...
window.renderCharacterTerminalData = function() {
    ...
    setVal('stat-charisma', c.stat_charisma || 'd4'); setVal('stat-dexterity', c.stat_dexterity || 'd4');
    setVal('stat-intelligence', c.stat_intelligence || 'd4'); setVal('stat-strength', c.stat_strength || 'd4');
    setVal('stat-toughness', c.stat_toughness || 'd4'); setVal('stat-willpower', c.stat_willpower || 'd4');
...
window.switchCodexCategory = function(cat) {
    ...
    const header = document.getElementById('codex-deck-title'); if (header) header.innerText = titles[cat] || 'Codex Archive';
    if (typeof renderCodexMatrix === 'function') renderCodexMatrix();
};

window.filterCodexEntries = function(val) { codexSearchFilter = (val || '').toLowerCase().trim(); if (typeof renderCodexMatrix === 'function') renderCodexMatrix(); };
...
function renderCodexMatrix() {
    ...
    if (codexSearchFilter) { entries = entries.filter(e => (e.title && e.title.toLowerCase().includes(codexSearchFilter)) || (e.subtitle && e.subtitle.toLowerCase().includes(codexSearchFilter)) || (e.content && e.content.toLowerCase().includes(codexSearchFilter))); }
        ...
        html += `
            <div class="codex-entry-card category-${e.category}">
                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                    <div><strong style="color:#00e5a3; font-size:13px; letter-spacing:1px;">${e.title}</strong><div style="font-size:10px; color:#6b826a; margin-top:2px;">${e.subtitle || 'General Record'}</div></div>
...
window.editCodexEntry = function(id) {
    ...
    document.getElementById('new-codex-title').value = entry.title || ''; document.getElementById('new-codex-subtitle').value = entry.subtitle || ''; document.getElementById('new-codex-content').value = entry.content || '';
    document.getElementById('new-codex-doc-name').value = entry.doc_name || ''; document.getElementById('new-codex-doc-data').value = entry.doc_data || ''; document.getElementById('new-codex-doc-type').value = entry.doc_type || '';

    const currentDocWrapper = document.getElementById('codex-current-doc-wrapper'); const currentDocName = document.getElementById('codex-current-doc-name');
    if (entry.doc_name && entry.doc_data) { currentDocWrapper.style.display = 'block'; currentDocName.innerText = `📎 ${entry.doc_name} (${(entry.doc_type || 'file').toUpperCase()})`; } else { currentDocWrapper.style.display = 'none'; }
    document.getElementById('btn-save-codex-entry').innerText = "✓ UPDATE CODEX ENTRY"; document.getElementById('btn-cancel-codex-edit').style.display = "block";
};

window.cancelCodexEdit = function() {
    editingCodexId = null;
    document.getElementById('codex-creator-heading').innerText = "+ New Codex Entry";
    document.getElementById('new-codex-title').value = ''; document.getElementById('new-codex-subtitle').value = ''; document.getElementById('new-codex-content').value = '';
    ...
    document.getElementById('btn-save-codex-entry').innerText = "+ PUBLISH TO CODEX"; document.getElementById('btn-cancel-codex-edit').style.display = "none";
};
...
window.saveNewCodexEntry = async function() {
    ...
    if (editingCodexId) {
        ...
        if (error) { alert("Failed to update Codex entry: " + error.message); } 
        else {
            window.cancelCodexEdit(); window.switchCodexCategory(cat); if (typeof loadCodexEntries === 'function') loadCodexEntries();
            await db.from('chat_logs').insert({ sender_id: currentUserId, content: `📖 [CODEX REVISED] Overseer modified archive: '${title}' under [${cat.toUpperCase()}].`, message_type: 'text' });
        }
    } else {
        const { error } = await db.from('codex_entries').insert(payload);
        if (error) { alert("Failed to publish to Cloud Codex: " + error.message); } 
        else {
            window.cancelCodexEdit(); window.switchCodexCategory(cat); if (typeof loadCodexEntries === 'function') loadCodexEntries();
            await db.from('chat_logs').insert({ sender_id: currentUserId, content: `📖 [CODEX PUBLISHED] Overseer filed archive: '${title}' under [${cat.toUpperCase()}].`, message_type: 'text' });
        }
    }
};
...
window.createOrUpdateNote = async function() {
    ...
    if (editingNoteId) {
        ...
        editingNoteId = null; document.getElementById('btn-create-note').innerText = "+ CREATE NOTE";
    } else {
...
window.editNote = function(id) {
    ...
    editingNoteId = id; document.getElementById('term-note-title').value = n.title; document.getElementById('term-note-content').value = n.content; document.getElementById('term-note-scope').value = n.share_scope;
        ...
        html += `
            <div class="note-card">
                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                    ...
                    ${isMine ? `<div style="display:flex; gap:4px;"><button class="layer-edit" onclick="window.editNote('${n.id}')" style="font-size:8px;">Edit</button><button class="layer-del" onclick="window.deleteNote('${n.id}')" style="font-size:8px;">X</button></div>` : ''}
                </div>
